...
def main():
    ...
    path = "/home/joeadamo/Research/CovA-NN-Emulator/Training-Set/"
    N = 101520
    idx = 0
    for i in range(N):
        filename = path + "CovA-"+f'{i:04d}'+".txt"
        # use a different prefix to prevent overwriting files
        new_filename = path + "Cov-"+f'{idx:05d}'+".txt"
    ...
    for j in range(N):
        filename = path + "Cov-"+f'{j:05d}'+".txt"
        new_filename = path + "CovA-"+f'{j:05d}'+".txt"
        if os.path.exists(filename):
            os.rename(filename, new_filename)
