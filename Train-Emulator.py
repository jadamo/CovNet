...
def main():
    ...
    print("Training VAE net: features net:    [" + str(do_main) + ", " + str(do_features) + "]")
    print("Saving to", save_dir)
    print("Using GPU:", torch.cuda.is_available())
    print("network structure flag =", structure_flag)

    if start_from_checkpoint == True:
        assert structure_flag == 4 or structure_flag == 2, "checkpointing only implimented for MLP-based networks!"

    batch_size = 200
    lr        = [1.438e-3, 1.438e-4, 1.438e-5]
    ...
    if start_from_checkpoint == True: 
        net.load_pretrained(checkpoint_dir+"network-VAE.params")
    ...
    for round in range(num_rounds):
        ...
        if do_main:
            ...
            if structure_flag != 2 and structure_flag != 4: 
                net.train_VAE(num_epochs, batch_size, BETA, \
                              optimizer, train_loader, valid_loader, \
                              True, save_dir, lr=lr[round])
            else: 
                net.train_MLP(num_epochs, batch_size, \
                              optimizer, train_loader, valid_loader, \
                              True, save_dir, lr=lr[round])
            t2 = time.time()
